...
| R3 | **User enumeration при регистрации** | После марта 2026: API и server actions возвращают единое «Пользователь с такими данными уже существует» для конфликта username/email. | Снижено (частично устранено) |
...
| # | Уязвимость | Описание | Критичность |
|---|------------|----------|------------|
| A1 | **Нет rate limiting на логин** | Логин через NextAuth `signIn("credentials")` без достаточного лимита на стороне маршрута (см. `checkLoginRateLimit` на форме — убедиться, что лимит покрывает все пути). Возможен перебор паролей по логину. | Высокая |
| A2 | **Длинный срок жизни сессии** | JWT и cookie сессии: `maxAge: 30 * 24 * 60 * 60` (30 дней). Увеличенное окно при компрометации токена. | Средняя |
| A3 | **Разные ответы при ошибке входа** | NextAuth `authorize`: «Пользователь не найден» vs «Неверный пароль» — позволяет перечислять зарегистрированные логины. | Средняя |
| A4 | **Логирование чувствительных данных** | В `registerUserAction` и др. в лог передаётся `{ name, phone }` при ошибке. В логах могут оказаться PII. | Низкая |
...
| O4 | **bcrypt rounds** | Регистрация в core: `registerUserWithCredentials` использует 12 раундов. Проверить `resetPasswordByToken` и прочие пути на единообразие. | Низкая |
...
2. **Унифицировать и усилить политику паролей**
   - В `authSchemas.ts`: увеличить минимальную длину (например, до 8), добавить требования: верхний/нижний регистр, цифра, спецсимвол (или явно зафиксировать осознанно слабые правила в политике безопасности).
   - Применить одну и ту же схему в API routes и Server Actions (shared schema).

3. **Устранение user enumeration при регистрации и логине**
...
7. **Ограничение раскрытия в восстановлении пароля**
...
9. **Единая валидация и срок сессии**
...
10. **bcrypt и токены**
    - Довести bcrypt до 12 во всех путях хеширования пароля (регистрация в core уже 12).
    - В API для `reset-password` ограничить длину token (например, `max(64)` или по длине UUID).

11. **Документация и политика**
