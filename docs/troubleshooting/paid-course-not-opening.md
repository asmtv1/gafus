...
1. **Webhook 403 + «Invalid signature»** — либо не совпадает `YOOKASSA_SECRET_KEY`, либо **заголовок подписи не доходит** до приложения. В логах смотреть: `hasSignature: false` значит ЮKassa не присылает `X-Yookassa-Signature` (типично при настройке через «Интеграция → HTTP-уведомления») или nginx его срезает — в коде сделан fallback: при отсутствии подписи принимаем webhook по IP whitelist. Если подпись есть, но не сходится — проверить, что на сервере тот же секретный ключ, что в кабинете ЮKassa (без пробелов/переносов в конце).
...
На проде: `docker logs gafus-web`. Искать сообщения: `Платёж подтверждён`, `Ошибка транзакции`, `payments/webhook`, `course-service`. Ошибки — в Tracer (apptracer.ru).
