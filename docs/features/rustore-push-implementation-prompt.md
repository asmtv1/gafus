...
> **Статус:** ✅ Реализовано. Актуальная документация: [rustore-push.md](./rustore-push.md).

Оригинальный промпт для агента (архив, для справки).

---

## Контекст

**Дистрибуция:** сейчас — только RuStore (Android) и App Store (iOS). Google Play планируется позже, но приложение пока туда не выкладывается.

**Важно:** Push-уведомления работают и без публикации в магазинах. FCM, RuStore Push и APNS работают с тестовыми/sideload-сборками — достаточно правильной конфигурации и нужных сервисов на устройстве.
...
**Решение:** Добавить RuStore Push для Android. iOS и Web — без изменений. Android: поддержать оба канала — RuStore (для устройств с RuStore/Почта/Дзен) и Expo/FCM (для устройств с GMS, включая будущих пользователей Google Play). Один пользователь может иметь подписки обоих типов — бекенд шлёт на все активные.
