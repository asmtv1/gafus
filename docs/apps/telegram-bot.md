...
- **📞 Смена телефона** - Пользователь запрашивает код в приложении (web/mobile), код отправляется в Telegram через Bot API (sendMessage). Подтверждение по коду в приложении.
- **👤 Смена логина** - После смены username в приложении пользователю в Telegram уходит уведомление о новом логине (sendMessage). Отправка через Telegram Bot API из packages/auth, не обязательно через процесс telegram-bot.
