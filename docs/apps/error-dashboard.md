...
**Проблема:**
- Удаление не работало из-за несовпадения `tag_*` labels между запросом на удаление и реальными labels в stream Loki
...
- Задержка 500ms была недостаточной для асинхронного удаления Loki
...
- Увеличена задержка после удаления с 500ms до 2 секунд для асинхронного удаления Loki
...
// Увеличена задержка для асинхронного удаления Loki
...
- Достаточная задержка для асинхронного удаления Loki
...
- `LokiClient.deleteLogs` принимает `startIso/endIso` и всегда отправляет в Loki корректные значения `start/end`, избегая `invalid start time`
...
- Улучшено логирование на всех уровнях (server action, LokiClient, UI)
...
Улучшенная валидация в `LokiClient.deleteLogs`:

```typescript
// Валидация базового URL
if (!this.baseUrl) {
  return { success: false, error: "Loki baseUrl не настроен" };
}
...
### Version 1.1.0 - Исправление удаления логов из Loki

**Дата:** 02 декабря 2024

**Изменения:**
- Добавлена переменная окружения `LOKI_URL` в production конфигурацию Docker
