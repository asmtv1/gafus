...
- Комментарий тренера к каждому вопросу (показывается пользователю после ответа, до 500 символов)
  ...
  checklist: [
    {
      ...
      correctAnswer: 0,
      comment: "Сфокусируйтесь на спокойной интонации и не забывайте про жест.",
    },
  ],
});
```
...
4. **Пользователь получает уведомление:**
   - При зачёте: заголовок `"[Название шага]" зачтён! ✅`, текст "Тренер проверил ваш экзамен. Можете переходить к следующему шагу."
   - При отклонении: заголовок `"[Название шага]" требует доработки ❗️`, текст включает комментарий тренера либо напоминание доработать отчёт.
...
const payload =
  action === "approve"
    ...
    : {
        ...
        body: comment ? `Комментарий тренера: ${comment}` : "Тренер вернул экзамен на доработку.",
      };
...
- `reject`: статус остается `IN_PROGRESS`, `isPassed` → `false`, **пользователь получает пуш с комментарием тренера** 🔔
...
- ✅ **Обновлён локальный nginx** - исправлен конфиг `/opt/homebrew/etc/nginx/servers/gafus-localhost.conf`
...
### Проблема: Очистка отладочного кода
...
export function ExamResultsListWithFilter({ initialExamResults, initialHideCompleted }) {
...
**Troubleshooting: Экзамены исчезают после зачёта**
...
_Система экзаменационных шагов обеспечивает комплексную проверку знаний и навыков пользователей с гибкой настройкой типов проверки._
