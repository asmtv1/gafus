...
**Обновление:** закрыты пункты **§1** (TracerProvider в admin-panel), **§2.1** (список web/trainer/admin + `packages/csrf` + `LoginForm` в ui-components), **§2.2** (mobile из списка), **§2.3–2.4** (zustand + pushStore), **§3** (серверные `logger.error` с `Error`, cdn-upload и др.), **§4** там, где добавлен `reportClientError` / мобильный аналог. **§6.3–6.4** (файлы без `try` / без `logger.error` в action) — по-прежнему кандидаты на ручной разбор, не автоматическое «исправление».
