...
## 🔐 Исправление аутентификации и редиректов

**Дата:** 26 октября 2025

### Проблема 1: Автоматический выход при закрытии вкладки

При закрытии вкладки `http://web.gafus.localhost` пользователя автоматически выкидывало из профиля и требовало повторного входа.

**Причина:** NextAuth создавал **session cookies** без указания `maxAge`, которые браузер автоматически удалял при закрытии вкладки/браузера.

**Решение:** Добавлен параметр `maxAge: 30 * 24 * 60 * 60` (30 дней) для всех authentication cookies в конфигурации NextAuth:
...
### Проблема 2: Отсутствие редиректа для залогиненных пользователей

Залогиненные пользователи могли оставаться на страницах `/`, `/login`, `/register` вместо автоматического перенаправления на `/courses`.

**Решение:** Улучшена логика middleware для автоматического редиректа залогиненных пользователей:
...
- ✅ **Пользователи остаются залогинеными** 30 дней
- ✅ **Не требуется повторный вход** при закрытии вкладки/браузера
- ✅ **Автоматический редирект** залогиненных пользователей на `/courses`
- ✅ **Защита от повторного входа** - залогиненный пользователь не может попасть на `/login` или `/register`
- ✅ **Соответствие стандартам UX** - пользователь сразу попадает в приложение
- ✅ **Безопасность сохранена** - `httpOnly`, `sameSite: 'lax'`, `secure` в production

### Файлы изменены

- `packages/auth/src/auth.ts` - добавлен `maxAge` для cookies
- `apps/web/src/middleware.ts` - улучшена логика редиректа
- `docs/packages/auth.md` - обновлена документация
