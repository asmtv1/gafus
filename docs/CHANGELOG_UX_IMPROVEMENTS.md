...
## ✅ Улучшение кнопки "Очистить кэш" для PWA (v2)

**Дата:** 8 ноября 2025

### Проблема

После предыдущих исправлений кнопка "Очистить кэш" все еще могла ломать PWA при перезапуске:

1. Пользователь нажимает "Очистить кэш" на странице профиля
2. Остается на странице профиля
3. Закрывает PWA
4. При открытии браузер пытается загрузить сложную страницу профиля (последнюю открытую)
5. Service Worker не успевает загрузить все данные
6. Показывается offline fallback, агрессивно пытающийся перезагрузиться

### Новое решение

Кнопка "Очистить кэш" теперь работает как **полный перезапуск приложения** - "спасательный круг" при критических ошибках.

#### 1. Автоматический редирект на главную

**Файл:** `apps/web/src/features/profile/components/SettingsActions.tsx`

После очистки кэша происходит автоматический редирект на главную страницу:
...
// Даем время на завершение операций очистки,
// затем делаем жесткий редирект на главную
setTimeout(() => {
  window.location.href = "/";
}, 800);
```

**Почему главная страница:**

- Загружается быстрее профиля (меньше серверных запросов)
- Проще структура данных
- Предсказуемое поведение для пользователя
- Меньше шансов застрять на offline fallback

#### 2. Увеличенные таймауты для главной страницы

**Файл:** `apps/web/public/sw.js`

Service Worker дает больше времени на загрузку главной страницы после очистки кэша:

```javascript
const isHomePage = reqUrl.pathname === "/";

if (timeSinceCleared < 2 * 60 * 1000) {
  // Для главной страницы используем больший таймаут
...
- Главная страница: 8 секунд (первая попытка) + 15 секунд (повторная)
- Другие страницы: 5 секунд + 10 секунд
- Действует 2 минуты после очистки кэша

#### 3. Менее агрессивная offline страница

**Файл:** `apps/web/public/sw.js`

Уменьшена частота автоматических попыток перезагрузки:

```javascript
// Было: каждые 5 секунд, до 3 попыток
// Стало: каждые 10 секунд, до 2 попыток
setInterval(() => {
...
1. **Четкий UX-паттерн** - Очистка кэша = перезапуск приложения
2. **Надежность** - Главная страница загружается быстрее и стабильнее
3. **Предсказуемость** - Пользователь всегда попадает на главную после очистки
4. **Меньше зависаний** - Нет застревания на офлайн странице
5. **"Спасательный круг"** - При любой критической ошибке: очистить кэш → все загрузится заново

### Тестирование

Обновлена документация: `docs/testing/CACHE_CLEAR_TEST.md`

**Основные сценарии:**

1. ✅ Очистка кэша → автоматический редирект на главную
2. ✅ Перезапуск PWA после очистки → главная загружается за 5-8 секунд
3. ✅ Очистка в медленной сети → редирект происходит, главная грузится до 15 секунд
4. ✅ Offline fallback менее агрессивен → попытки каждые 10 секунд (макс. 2)

### Технические детали

**Измененные файлы:**

- `apps/web/src/features/profile/components/SettingsActions.tsx` - редирект на главную
- `apps/web/public/sw.js` - увеличенные таймауты для главной (8/15 сек)
- `apps/web/public/sw.js` - offline fallback (10 сек между попытками)
- `docs/testing/CACHE_CLEAR_TEST.md` - обновлены тесты и инструкции
...
## 🔧 Исправление проблемы с очисткой кэша в PWA

**Дата:** 6 ноября 2025

### Проблема

При очистке кэша в PWA на iPhone:

1. Пользователь нажимает "Очистить кэш"
2. Закрывает PWA
3. При повторном открытии PWA показывается страница "Нет подключения к интернету"
4. Кнопки "Назад" и "Попробовать снова" не работают

### Причина

После очистки кэша Service Worker удалял все кэшированные HTML-страницы. При перезапуске PWA:

1. Service Worker пытался загрузить страницу из кэша - кэша нет
2. Пытался загрузить из сети с таймаутом 1200ms
3. На медленном соединении или из-за особенностей iOS таймаут истекал
4. Показывалась офлайн-страница без возможности восстановления
...
Добавлен флаг в localStorage и IndexedDB при очистке кэша:

```typescript
// Устанавливаем флаг для Service Worker
localStorage.setItem("cache-cleared-timestamp", Date.now().toString());

// Сохраняем также в IndexedDB для доступа из Service Worker
const request = indexedDB.open("sw-localstorage", 1);
// ... сохранение флага
```

#### 2. Увеличенный таймаут после очистки

**Файл:** `apps/web/public/sw.js`

Service Worker проверяет флаг и увеличивает таймаут:

```javascript
// Проверяем, был ли недавно очищен кэш
...
if (cacheCleared) {
  ...
  if (timeSinceCleared < 2 * 60 * 1000) {
    networkTimeout = 5000; // 5 секунд вместо 1200ms
    shouldRetry = true;
  }
}
```

#### 3. Повторная попытка загрузки

Если первая попытка не удалась, Service Worker делает вторую попытку с 10-секундным таймаутом:

```javascript
try {
  response = await Promise.race([fetch(request), timeout(networkTimeout)]);
} catch (firstError) {
  if (shouldRetry && firstError.message === "timeout") {
    // Вторая попытка с 10 секундами
    response = await Promise.race([fetch(request), timeout(10000)]);
  }
}
```

#### 4. Улучшенная офлайн-страница

**Файл:** `apps/web/public/sw.js`

Добавлена умная офлайн-страница:

- ✅ Кнопка "Попробовать снова" с параметром `?retry=timestamp` для обхода кэша
- ✅ Кнопка "Назад" с fallback на главную страницу
- ✅ Кнопка "На главную" для прямого перехода
- ✅ Автоматическая попытка подключения каждые 5 секунд при обнаружении онлайн-статуса
- ✅ Заголовок `Cache-Control: no-cache` для предотвращения кэширования офлайн-страницы

#### 5. Утилиты для IndexedDB

Добавлены функции для работы с IndexedDB из Service Worker:

- `getLocalStorageItem(key)` - чтение значения
- `setLocalStorageItem(key, value)` - запись значения
- `removeLocalStorageItem(key)` - удаление значения

### Преимущества

- ✅ Корректная работа после очистки кэша на iPhone
- ✅ Увеличенные таймауты для медленных соединений
- ✅ Автоматическое восстановление при появлении интернета
- ✅ Улучшенный UX с работающими кнопками навигации
- ✅ Нет "застревания" на офлайн-странице
...
#### 3. Создан новый компонент TrainingReminders
...
2. Добавлено поле `name` и убран unique constraint: `20251026204225_update_reminder_add_name_remove_unique`
...
export const metadata = generateStaticPageMetadata("Заголовок", "Описание", "/path");
...
```typescript
export const metadata = generateStaticPageMetadata("Список курсов", "...", "/courses");
```
...
#### 4. Добавлен функционал тестирования
...
- **Вирусность**: красивое превью увеличивает шанс перехода по ссылке
...
```typescript
const urlPattern =
  /^https?:\/\/(www\.|m\.)?(youtube\.com|youtu\.be|rutube\.ru|vimeo\.com|vk\.com)\/.+/;
```
...
## ⏱ Оценочное время для теоретических и экзаменационных шагов

**Дата:** 21 ноября 2025

### Обзор

Добавлена поддержка отдельного оценочного времени прохождения шага (в минутах) для теоретических (`THEORY`) и экзаменационных (`EXAMINATION`) шагов. Это время не связано с таймером и показывается пользователю отдельно, чтобы не путать его с фактическим отсчётом.

### ✨ Реализованные изменения

- В модели `Step` и `StepTemplate` добавлено поле `estimatedDurationSec`:
  - `durationSec` — фактическая длительность для таймера тренировочного шага (`TRAINING`).
  - `estimatedDurationSec` — оценочное время прохождения шага для отображения (`THEORY`, `EXAMINATION`, опционально `TRAINING`).
- В панели тренера:
  - Форма шага `NewStepForm` получила поле «Примерное время (минуты)»:
    - Для тренировочных шагов — необязательное поле.
    - Для теоретических и экзаменационных шагов — используется только как оценка, без таймера.
  - Данные сохраняются через server actions `createStep` и `updateStep` в `estimatedDurationSec`.
  - Таблица шагов `EnhancedStepsTable` показывает отдельную колонку «Оценка (сек)».
- В пользовательском веб‑приложении:
  - В списке дней тренировок (`TrainingDayList`) оценка дня теперь считается с учётом:
    - `durationSec` для тренировочных шагов.
    - `estimatedDurationSec` для теоретических и экзаменационных шагов.
  - На странице дня тренировки (`Day` / `AccordionStep`) для теоретических и экзаменационных шагов добавлен отдельный бейдж:
    - Под заголовком шага отображается строка вида `Примерное время: ~ N мин` отличным цветом, чтобы разделять оценку и таймер.

### 📂 Файлы

- `packages/prisma/schema.prisma`
- `packages/types/src/data/training.ts`
- `packages/types/src/utils/training.ts`
- `packages/types/src/components/trainer-panel.ts`
- `apps/trainer-panel/src/features/steps/components/NewStepForm.tsx`
- `apps/trainer-panel/src/features/steps/lib/createStep.ts`
- `apps/trainer-panel/src/features/steps/lib/updateStep.ts`
...
- `apps/web/src/features/training/components/Day.tsx`
- `apps/web/src/features/training/components/AccordionStep.tsx`
- `apps/web/src/features/training/components/AccordionStep.module.css`

---

## 🐛 Исправление ошибки 500 на главной странице
...
Также переведены на использование `getCurrentUserIdFromAuth()` для избежания лишних исключений.
...
- Добавлена звездочка (_) в заголовок "Логотип курса _"
...
## 📋 Интеграция Bull Board в Error Dashboard
...
```typescript
import { hapticStart, hapticComplete, hapticLight } from "@/utils/hapticFeedback";
...
_Документ актуален на 14 октября 2025_
