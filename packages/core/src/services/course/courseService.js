...
export async function getCoursesWithProgress(userId) {
  ...
  return allCourses.map((course) => {
    ...
    const userStatus = userCourse?.status ? userCourse.status : TrainingStatus.NOT_STARTED;
    return {
      id: course.id,
      name: course.name,
      type: course.type,
 ...
 */
export async function checkCourseAccess(courseType, userId) {
 ...
 */
export async function checkCourseAccessById(courseId, userId) {
  if (!userId) {
    const course = await prisma.course.findUnique({
      where: { id: courseId },
      select: { isPrivate: true },
 ...
 */
export async function getCourseMetadata(courseType) {
