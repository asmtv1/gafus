...
async function setupDashboards() {
  ...
  if (!SEQ_API_KEY) {
    ...
    console.log('   4. Установите разрешение "Ingest" (минимум, для полного доступа можно все разрешения)');
  ...
  try {
    ...
    try {
      ...
      if (error.message.includes('401') || error.message.includes('403')) {
        ...
        console.log(`   💡 Запустите: docker-compose -f ci-cd/docker/docker-compose.prod.yml up -d seq\n`);
        printManualInstructions();
        process.exit(1);
      } else {
    ...
    if (error.message.includes('ECONNREFUSED')) {
      ...
      console.error('   Можно запустить: docker-compose -f ci-cd/docker/docker-compose.prod.yml up -d seq\n');
    } else if (error.message.includes('404')) {
