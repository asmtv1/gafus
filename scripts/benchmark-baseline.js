...
class PerformanceBenchmark {
  ...
  analyzeBundleSize() {
    ...
    try {
      ...
      this.success(`Общий размер bundle: ${Math.round(totalSize / 1024 / 1024)}MB (${fileCount} файлов)`);
   ...
   */
  saveResults() {
    const outputPath = path.join(process.cwd(), '.cursor/plans/BASELINE_МЕТРИКИ_ПРОИЗВОДИТЕЛЬНОСТИ.json');
