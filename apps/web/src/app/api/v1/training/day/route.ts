...
export async function GET(request: NextRequest) {
  try {
    ...
    // Динамический импорт
    const { getTrainingDayWithUserSteps } = await import("@shared/lib/training/getTrainingDayWithUserSteps");
    ...
    if (!result) {
      return NextResponse.json({ success: false, error: "День тренировки не найден", code: "NOT_FOUND" }, { status: 404 });
    }
