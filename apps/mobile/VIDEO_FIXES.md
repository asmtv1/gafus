...
### Fullscreen на mobile (expo-video)
- В полноэкранном режиме кастомная панель (пауза, выход) не отображается — в fullscreen показывается только VideoView.
- **Решение**: включены нативные контролы (`nativeControls={true}`), чтобы в fullscreen на iOS/Android отображались системные кнопки паузы и выхода из полноэкранного режима.

### Дополнительные возможности плеера
- **Скорость воспроизведения**: кнопка 1× в панели переключает 1 → 1.25 → 1.5 → 1.75 → 2× (удобно для обучающих видео).
- **Индикатор буфера**: на прогресс-баре отображается серая полоса — сколько секунд уже загружено (Android/iOS из `timeUpdate.bufferedPosition`).
- **Double-tap перемотка**: двойной тап по левой части видео −10 сек, по правой +10 сек (как в YouTube); одиночный тап — play/pause.

### Чеклист:
- [ ] CDN видео загружается и получает signed URL
- [ ] Видео начинает воспроизведение
- [ ] Контролы работают (play/pause, seek, fullscreen), в т.ч. выход из fullscreen и пауза
