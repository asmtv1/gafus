...
function StackTraceSection({ stack, onCopy }: { stack: unknown; onCopy?: (text: string) => void }) {
  ...
  return (
    <Paper
      elevation={0}
      sx={{ p: 3, mb: 3, bgcolor: "#0f0f0f", borderRadius: 2, border: "1px solid #333" }}
    >
      ...
      if (componentMatch) {
        ...
        return (
          <Box key={index} component="span" sx={{ display: "block" }}>
            ...
            <Box component="span" sx={{ color: "#60a5fa", fontWeight: "bold" }}>
              {componentName}
            </Box>
            <Box component="span" sx={{ color: "#e2e8f0" }}>
              {line.substring(line.indexOf(componentName) + componentName.length)}
            </Box>
            {"\n"}
          </Box>
        );
...
function UserAgentSection({ userAgent }: { userAgent: unknown }) {
  ...
  return (
    <Paper
      elevation={0}
      sx={{ p: 3, mb: 3, bgcolor: "#f7fafc", borderRadius: 2, border: "1px solid #e2e8f0" }}
    >
...
function AdditionalContextSection({
  ...
  return (
    <Paper
      elevation={0}
      sx={{ p: 3, mb: 3, bgcolor: "#1a1a0a", borderRadius: 2, border: "1px solid #3d3d00" }}
    >
      <Box display="flex" alignItems="center" justifyContent="space-between" mb={2}>
        <Typography
          variant="h6"
...
export default function ErrorDetailsModal({
  ...
  const handleCopyForAI = async () => {
    try {
      ...
      setSnackbar({
        open: true,
        message: "Скопировано для AI! Вставьте в чат.",
        severity: "success",
      });
    } catch (err) {
    ...
    >
      <DialogTitle
        sx={{
          ...
          borderBottom: isFatal ? `2px solid ${severityColor}` : "1px solid #dee2e6",
        }}
      >
      ...
      <DialogContent sx={{ p: 3, overflow: "auto" }}>
        <Paper elevation={0} sx={{ p: 3, mb: 3, bgcolor: "#fafafa", borderRadius: 2 }}>
          ...
          <Box display="flex" flexWrap="wrap" gap={1} mb={2}>
            ...
            <Chip label={error.environment} size="small" variant="outlined" />
          ...
          {error.status === "resolved" && error.resolvedAt && (
            <Box mt={2}>
              <Typography variant="body2" color="text.secondary">
                <CheckCircleIcon
                  fontSize="small"
                  sx={{ verticalAlign: "middle", mr: 0.5, color: "#2e7d32" }}
                />
                Решена{" "}
        ...
        <Paper
          elevation={0}
          sx={{ p: 3, mb: 3, bgcolor: "#fff9e6", borderRadius: 2, border: "1px solid #ffd54f" }}
        >
        ...
        <Paper
          elevation={0}
          sx={{ p: 3, mb: 3, bgcolor: "#edf2f7", borderRadius: 2, border: "1px solid #cbd5e0" }}
        >
          ...
          <Box display="grid" gridTemplateColumns={{ xs: "1fr", sm: "repeat(2, 1fr)" }} gap={2}>
            ...
            <Box>
              ...
              <Typography variant="body2" color="text.secondary" sx={{ fontFamily: "monospace" }}>
                {error.updatedAt || error.createdAt
        ...
        <Paper
          elevation={0}
          sx={{ p: 3, mb: 3, bgcolor: "#f0f9ff", borderRadius: 2, border: "1px solid #7dd3fc" }}
        >
        ...
        <Paper
          elevation={0}
          sx={{ p: 3, mb: 3, bgcolor: "#f0fff4", borderRadius: 2, border: "1px solid #68d391" }}
        >
        ...
        {error.tags && error.tags.length > 0 && (
          <Paper
            elevation={0}
            sx={{ p: 3, mb: 3, bgcolor: "#fdf2f8", borderRadius: 2, border: "1px solid #f9a8d4" }}
          >
      ...
      <DialogActions sx={{ p: 3, pt: 0, flexWrap: "wrap", gap: 1 }}>
        <Box sx={{ display: "flex", gap: 1, flexWrap: "wrap", flex: 1 }}>
          ...
          <Tooltip title="Копировать как JSON">
            <Button onClick={handleCopyJSON} variant="outlined" startIcon={<CopyIcon />}>
              JSON
            </Button>
          </Tooltip>

          <Tooltip title="Скачать как JSON файл">
            <Button onClick={handleDownloadJSON} variant="outlined" startIcon={<DownloadIcon />}>
              Скачать JSON
            </Button>
          </Tooltip>
