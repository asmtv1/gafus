...
export async function invalidateTrainingDayCache(dayId: string, skipCoursesInvalidation?: boolean) {
  try {
    ...
    if (skipCoursesInvalidation) {
      ...
      logger.warn(`[Cache] Training day ${dayId} cache invalidated successfully`, {
        operation: "warn",
      });
      return;
    }
    ...
    try {
      ...
      if (courseIds.length > 0) {
        logger.warn(
          `[Cache] Found ${courseIds.length} courses using day ${dayId}, invalidating their cache`,
          {
            dayId,
            courseIds,
            operation: "warn",
    ...
    logger.error("❌ Error invalidating training day cache:", error as Error, {
      operation: "error",
    });
    logger.error(
