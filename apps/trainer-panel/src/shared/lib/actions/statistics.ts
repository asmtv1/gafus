...
export async function getDetailedCourseStatisticsAction(courseId: string) {
  try {
    ...
    logger.error("Error fetching detailed course statistics:", error as Error, { operation: 'error' });
...
export async function getDetailedStepStatisticsAction(stepId: string) {
  try {
    ...
    logger.error("Error fetching detailed step statistics:", error as Error, { operation: 'error' });
    return { success: false, error: "Внутренняя ошибка сервера" };
  }
}
